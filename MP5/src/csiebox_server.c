...
void sig_handler(int signo)
{
		if (signo == SIGUSR1){
				...
				int ffd = open(fifoname, O_WRONLY | O_TRUNC);
				uint32_t tmp = active_thread_num;
				htonl(tmp);
				write(ffd, &tmp, sizeof(uint32_t));
		}
...
void csiebox_server_init(csiebox_server** server, int argc, char** argv) {
	...
	*server = tmp;
	...
	sprintf(pid_file, "%s/csiebox_server.pid", (*server)->arg.run_path);
	int pidfile = open(pid_file, O_WRONLY | O_TRUNC | O_CREAT);
	char XD[20] = "\0";
	sprintf(XD, "%d", my_pid);
	write(pidfile, XD, strlen(XD));
	memset(fifoname, 0, sizeof(fifoname));
	sprintf(fifoname, "%s/fifo.%d", (*server)->arg.run_path, my_pid);
